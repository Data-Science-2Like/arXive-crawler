...
def get_proxy_list():
    ...
    return df[(df['Https'] == 'yes') & (df['Country'] == 'Germany')]
    ...
    try:
        while requestCount < len(metaData):
            ...
            async with aiohttp.ClientSession() as session:
                ret = await asyncio.gather(*[downloadPaper(dest, url, id, session, curr_proxy) for (url, id) in currUrls])
...
async def downloadPaper(dest, url, identifier, session, proxy):
    ...
    try:
        if len(proxy) > 4:
            async with session.get(url=url, proxy=proxy, max_redirects=10, allow_redirects=True) as response:
